...
import {
  articleCardClass, articleMeta, articleTitle,
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {users.map((user) => (
        <div key={user._id} className={`${articleCardClass} flex flex-col`}>
          ...
          <span className={`text-xs font-medium mt-1 ${user.isUserActive ? "text-green-500" : "text-red-500"}`}>
            {user.isUserActive ? "ACTIVE" : "INACTIVE"}
          </span>
          <button className={`${ghostBtn} mt-auto pt-4`} onClick={() => toggleUser(user)}>
